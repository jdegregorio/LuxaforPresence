...
7. Fetch and fast-forward local `main` to the exact merge commit and wait for the post-merge `main` workflow to pass. When the application version changed, also verify that the matching GitHub Release contains `LuxaforPresence.dmg` and `LuxaforPresence.dmg.sha256`. End with `git status` clean and local `HEAD` equal to `origin/main`.
