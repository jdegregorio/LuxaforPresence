...
final class PresenceEngineTests: XCTestCase {
    ...
    func test_recentVoiceAfterCommunicationContextEnds_remainsVoiceRecent() {
        let harness = PresenceEngineHarness()
    ...
    func test_voiceAtRecentBoundary_returnsVoiceCooldown() {
        ...
        XCTAssertEqual(harness.outputs, [.solid(.orange)])
    ...
    func test_communicationContextEnding_preservesRecentAndCooldownTimeline() {
        let harness = PresenceEngineHarness()
        harness.meetingDetector.isActive = true
        harness.micCam.microphoneActive = true
        harness.voiceActivity.lastActivityDate = harness.currentDate
        ...
        tickAndWait(engine)
        harness.currentDate = harness.currentDate.addingTimeInterval(299)
        tickAndWait(engine)
        harness.currentDate = harness.currentDate.addingTimeInterval(300)
        tickAndWait(engine)

        XCTAssertEqual(
            harness.states,
            [.voiceRecent, .voiceCooldown, .available]
        )
        XCTAssertEqual(
            harness.luxafor.actions,
            [
                .red(harness.config.remoteWebhookUserId),
                .orange(harness.config.remoteWebhookUserId),
                .off(harness.config.remoteWebhookUserId),
            ]
        )
        XCTAssertEqual(
            harness.voiceActivity.lastVoiceActivityDate,
            qualifiedVoiceDate
        )
        XCTAssertEqual(harness.voiceActivity.captureContextChanges, [true, false])
    }

    func test_newCommunicationContextDuringTimeline_preservesPriorSignalState() {
        let harness = PresenceEngineHarness()
        harness.meetingDetector.isActive = true
        harness.micCam.microphoneActive = true
        harness.voiceActivity.lastActivityDate = harness.currentDate
    ...
    func test_qualifiedVoiceObservedAfterContextEnds_drivesLaterZoomSessionTimeline() {
        let harness = PresenceEngineHarness()
        let qualifiedVoiceDate = harness.currentDate
    ...
    func test_manualOverrides_sendConfiguredColorForEveryOperationalState() {
        let available = LuxaforColor(red: 1, green: 2, blue: 3)
        let quiet = LuxaforColor(red: 4, green: 5, blue: 6)
        let recent = LuxaforColor(red: 7, green: 8, blue: 9)
        let cooldown = LuxaforColor(red: 10, green: 11, blue: 12)
        let harness = PresenceEngineHarness { config in
            config.availableColor = available
            config.zoomQuietColor = quiet
            config.recentVoiceColor = recent
            config.voiceCooldownColor = cooldown
        }
        let engine = harness.makeEngine()

        engine.force(.available)
        engine.force(.zoomQuiet)
        ...
        XCTAssertEqual(
            harness.luxafor.actions,
            [
                .custom(available, harness.config.remoteWebhookUserId),
                .custom(quiet, harness.config.remoteWebhookUserId),
                .custom(recent, harness.config.remoteWebhookUserId),
                .custom(cooldown, harness.config.remoteWebhookUserId),
            ]
        )
        XCTAssertEqual(
            harness.outputs,
            [.solid(available), .solid(quiet), .solid(recent), .solid(cooldown)]
        )
    }
    ...
    func test_localHeartbeat_reassertsSolidRed() {
        let harness = PresenceEngineHarness()
        harness.meetingDetector.isActive = true
        harness.micCam.microphoneActive = true
        harness.voiceActivity.lastActivityDate = harness.currentDate
    ...
    func test_clearSignalTimeline_withActiveZoom_returnsToYellow() {
        let harness = PresenceEngineHarness()
        harness.meetingDetector.isActive = true
        harness.micCam.microphoneActive = true
        harness.voiceActivity.lastActivityDate = harness.currentDate
    ...
    func test_clearSignalTimeline_withMicrophoneOnly_returnsToOff() {
        let harness = PresenceEngineHarness()
        harness.micCam.microphoneActive = true
        harness.voiceActivity.lastActivityDate = harness.currentDate
        let engine = harness.makeEngine()
    ...
    func test_sleepWake_reevaluatesBeforeRestartingAndReassertingOutput() {
        ...
        XCTAssertEqual(engine.desiredOutput, .solid(.orange))
        XCTAssertEqual(
            harness.luxafor.actions,
            [
                .red(harness.config.remoteWebhookUserId),
                .forced(.orange, harness.config.remoteWebhookUserId),
            ]
        )
    ...
    func test_beginOutputRetirement_discardsInFlightSignalResult() {
        let harness = PresenceEngineHarness()
        let detector = BlockingMeetingDetector(result: true)
        let engine = harness.makeEngine(meetingDetector: detector)
        let tickCompleted = expectation(description: "retired poll completed")

        engine.tick { tickCompleted.fulfill() }
        XCTAssertEqual(detector.waitUntilStarted(), .success)
...
private final class FakeVoiceActivitySignal: VoiceActivitySignalProtocol {
    var authorizationState: MicrophoneAuthorizationState = .authorized
    ...
    func setCaptureContextActive(_ active: Bool) {
        setCaptureContextActive(active, minimumActiveDuration: 0.25)
    }
