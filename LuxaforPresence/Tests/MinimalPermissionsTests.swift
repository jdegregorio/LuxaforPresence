...
final class MinimalPermissionsTests: XCTestCase {
    func test_infoPlist_declaresOnlyMicrophonePrivacyUsage() throws {
        ...
        let releaseVersion = try XCTUnwrap(info["CFBundleShortVersionString"] as? String)
        let buildVersion = try XCTUnwrap(info["CFBundleVersion"] as? String)
        XCTAssertEqual(buildVersion, releaseVersion)
        XCTAssertNotNil(
    ...
    func test_bundledConfiguration_omitsUnusedDetectorKeys() throws {
        ...
        XCTAssertEqual(values["recentVoiceColor"] as? String, "#FF0000")
        XCTAssertEqual(values["voiceCooldownColor"] as? String, "#FF8C00")
